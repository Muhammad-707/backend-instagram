...
> - **Репост ≠ `Share`** (переделано 2026-07-20): у кнопки «двойная стрелка» своя модель `Repost` — это
>   состояние (toggle, `@@unique([postId, userId])`), а `Share` остался событием «отправил в чат / скопировал
>   ссылку». `GET /profile/{me,:userId}/reposts` читает `Repost`, `POST /posts/{id}/repost` его переключает.
>   Свой пост репостить нельзя (400). Порядок вкладки — время репоста, поэтому курсор — id репоста, не id поста.
